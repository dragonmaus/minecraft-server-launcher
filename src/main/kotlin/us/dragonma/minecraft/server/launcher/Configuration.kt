...
internal class Configuration(configFileName: String) {
            ...
            return try {
                val json =
                    URI("https://meta.fabricmc.net/v2/versions/installer")
                        ...
                        .toString()
                val data =
                    lazyJson.decodeFromString(
                        ListSerializer(FabricApiInstallerVersion.serializer()),
                        json,
                    )
    ...
    private val minecraftFabricLoaderVersion: String
        get() {
            return try {
                ...
                val data =
                    lazyJson.decodeFromString(
                        ListSerializer(FabricApiLoaderVersion.serializer()),
                        json,
                    )
    ...
    private val minecraftQuiltInstallerVersion: String
        get() {
            return try {
                val json =
                    URI("https://meta.quiltmc.org/v3/versions/installer")
                        .getText()
                        .body()
                        .toString()
                val data =
                    lazyJson.decodeFromString(
                        ListSerializer(QuiltApiInstallerVersion.serializer()),
                        json,
                    )
    ...
    private val minecraftQuiltLoaderVersion: String
        get() {
            return try {
                val json =
                    URI("https://meta.quiltmc.org/v3/versions/loader/$minecraftVersion")
                        .getText()
                        .body()
                        .toString()
                val data =
                    lazyJson.decodeFromString(
                        ListSerializer(QuiltApiLoaderVersion.serializer()),
                        json,
                    )
    ...
    @Serializable private data class ForgeApiVersion(val result: List<Map<String, List<String>>>)

    @Serializable private data class MinecraftApiVersion(val result: List<String>)

    @Serializable
    private data class NeoForgeApiVersion(val isSnapshot: Boolean, val version: String)
    ...
    @Serializable
    private data class QuiltApiLoaderVersionHashed(val maven: String, val version: String)

    @Serializable
    private data class QuiltApiLoaderVersionIntermediary(val maven: String, val version: String)
    ...
    @Serializable
    private data class QuiltApiLoaderVersionLauncherMetaLibrary(val name: String, val url: String)
...
internal data class MinecraftNeoForgeConfig(val version: String)
